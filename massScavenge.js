...
if (localStorage.getItem("maxUnits") == null) {
    console.log("No max units set, creating")
    var maxUnits = {
        "spear": 0,
        "sword": 0,
        "axe": 0,
    ...
    localStorage.setItem("maxUnits", JSON.stringify(maxUnits));
}
else {
    console.log("Grabbing max units setting");
    var maxUnits = JSON.parse(localStorage.getItem("maxUnits"));
}
...
for (var i = 0; i < sendOrder.length; i++) {
    ...
    <tbody>    
        ...
        <tr>
            <td style="text-align:center; width:auto; background-color:#202225;padding: 5px;"><font color="#ffffdf">Max units</font></td>
        </tr>
        <tr>
            <td align="center" style="background-color:${backgroundColor};padding: 5px;"><input type="text" ID="${sendOrder[i]}MaxUnits" name="${sendOrder[i]}" value="${maxUnits[sendOrder[i]]}" size="5"></td>
        </tr>
        </tbody>  
    </table>
</td>`);
...
// Make readyToSend globally accessible
...
function resetSettings() {
    ...
    localStorage.removeItem("maxUnits");
    UI.BanneredRewardMessage("Settings reset");
    window.location.reload();
}

// Make resetSettings globally accessible
